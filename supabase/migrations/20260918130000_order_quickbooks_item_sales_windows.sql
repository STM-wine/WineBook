-- Offset pagination is only safe when the RPC has a deterministic row order.
-- Keep the existing sales-window boundaries and sort by the grouped item keys.
...
order by item_list_id, item_full_name;
$$;
