...
async function fetchOrderingMarkers(supabase: SourceClient) {
  try {
    return await fetchAll<SourceOrderingMarker>(
      ...
      "item_code,quickbooks_item_list_id,is_btg,is_core,replenishment_policy,policy_family_key,policy_family_name,family_default_policy,recommendations_suppressed,note_source",
      "item_code"
    );
  } catch (error) {
