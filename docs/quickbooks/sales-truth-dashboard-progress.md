...
- After several clean one-week YTD recovery runs, added an idempotent consolidation step that merges only still-pending current-year `weekv3:*` sales-truth rows into two-week `span2v1:*` jobs. Already completed weekly windows stay intact, while the remaining YTD sales pulls move faster without switching to risky month-sized requests.
- After validating the Sales Dashboard on large YTD ranges, changed 2025 sales recovery to monthly `monthv1:*` invoice and credit memo checkpoints, newest month first. Still-pending 2025 `weekv3:*` and `span2v1:*` rows are marked completed/superseded so manual Web Connector runs can validate 2025 month by month.
